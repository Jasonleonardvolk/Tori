// ALAN IDE – ConceptFieldCanvas with Visual Encodings (Story 1.3)
// Author: Cascade (2025-05-07)
// Features: phase (color), entropy (overlay), usage (size), coupling (edge weight), animated overlays

import React, { useEffect, useRef, useState } from "react";
import useKappaGeometryLayout from "./useKappaGeometryLayout";
import * as d3 from "d3";

export default function ConceptFieldCanvasVisualEncodings({
  width = 1000,
  height = 700,
  ...
  return (
    ...
    >
      <g id="zoom-group">
        ...
        {/* Nodes: phase (color), usage (size), entropy (overlay) */}
        {graph.nodes.map(n => (
          <g key={n.id}>
            ...
            {/* Animated resonance pulse (optional, for future) */}
          </g>
        ))}
...
function edgeColor(coupling) {
  // Stronger coupling = more vibrant teal, weaker = gray
...
function entropyOverlay(entropy) {
  // High entropy = magenta/pink, low = transparent
