...
def demo_auto_prefill():
    ...
    # Check if concept databases exist
    ...
    if concept_db_path.exists():
        with open(concept_db_path, 'r') as f:
            main_db = json.load(f)
        print(f"📊 Main concept database: {len(main_db)} concepts")
    else:
        print("⚠️ Main concept database not found")
        main_db = []
    
    if seed_db_path.exists():
        with open(seed_db_path, 'r') as f:
            seed_db = json.load(f)
        print(f"🌍 Universal seed database: {len(seed_db)} concepts")
        ...
        print("\n🌍 Seed database domain coverage:")
        for domain, count in sorted(domains.items()):
            print(f"  {domain}: {count} concepts")
            
    else:
        print("⚠️ Universal seed database not found")
        seed_db = []
...
if __name__ == "__main__":
    ...
    print("3. Watch your concept database grow automatically!")
