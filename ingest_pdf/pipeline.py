...
concept_database, concept_names, concept_scores = [], [], {}
...
def load_universal_concept_database():
    ...
    concept_scores = {c["name"]: c.get("priority", 0.5) for c in concept_database}
    logger.info(f"🌍 UNIVERSAL DATABASE READY: {len(concept_database)} total concepts (+{seeds_added} seeds)")
...
def boost_known_concepts(chunk: str) -> List[Dict[str, Any]]:
    ...
    for concept in concept_database[:300]:  # Limit for performance
        ...
        if name.lower() in chunk_lower:
            ...
            boosted.append({
                "name": name,
                "score": boosted_score,
                "method": "database_boosted",
                "source": {"database_matched": True},
...
def ingest_pdf_clean(pdf_path: str, doc_id: str = None, extraction_threshold: float = 0.0, admin_mode: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        response = {
            ...
            "database_boosted": boosted_count,
