...
def video_to_concept_database(video_path: str, auto_prefill: bool = True) -> Dict[str, Any]:
    """
    Process video and optionally add concepts to database
    
    Args:
        video_path: Path to video file
        auto_prefill: Whether to auto-add high-quality concepts to database
        
    Returns:
        Processing results with concept statistics
    """
    logger.info(f"🎬 Processing video for concept database: {Path(video_path).name}")
    ...
    if auto_prefill:
        try:
            ...
            # Add high-quality video concepts to database
            prefilled_count = auto_prefill_concept_db(concepts, Path(video_path).name)
            
            logger.info(f"📥 Auto-prefilled {prefilled_count} video concepts to database")
            
        except ImportError:
            logger.warning("⚠️ Auto-prefill not available - concepts not added to database")
            prefilled_count = 0
    else:
        prefilled_count = 0
