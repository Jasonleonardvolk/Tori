...
mkdir -p logs/ingest-bus

# Start the service
echo "Starting Ingest Bus service on port 8080..."
cd ingest-bus
python3 -m uvicorn main:app --reload --host 0.0.0.0 --port 8080 --log-level info > ../logs/ingest-bus/service.log 2>&1 &

# Save PID for later management
echo $! > ../logs/ingest-bus/service.pid
echo "Service started with PID: $(cat ../logs/ingest-bus/service.pid)"
echo "Logs available at: $(pwd)/../logs/ingest-bus/service.log"
