...
color 0C
echo.
echo  PRAJNA PDF-ONLY FEEDING SYSTEM - WORKING VERSION
echo ==================================================
echo.
...
if exist "prajna_pdf_knowledge.json" (
    ...
    python -c "import json; data=json.load(open('prajna_pdf_knowledge.json')); print('Total PDFs:', len(data.get('pdf_documents', []))); print('Total Pages:', sum(doc.get('pages', 0) for doc in data.get('pdf_documents', []))); meta=data.get('metadata', {}); print('Last Updated:', meta.get('processing_date', 'Unknown')[:19] if meta.get('processing_date') else 'Unknown');"
) else (
