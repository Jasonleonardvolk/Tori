...
// Lets overlays dynamically update the map and legendarium as new events unfold
import React, { useEffect } from 'react';

export default function DynamicOverlaySync({ content, onClose, updateMap, updateLegendarium, event }) {
  useEffect(() => {
    if (event) {
      if (updateMap) updateMap(event);
      if (updateLegendarium) updateLegendarium(event);
    }
  }, [event, updateMap, updateLegendarium]);
