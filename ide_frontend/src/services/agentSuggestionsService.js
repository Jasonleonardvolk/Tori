...
class AgentSuggestionsService {
  ...
  async fetchSuggestions(context) {
    ...
    // Use new socialImpactScorer
    return raw.map(s => {
      const { classification, score, rationale } = require('./socialImpactScorer.js').scoreSocialImpact(s);
      return {
        ...s,
        socialImpact: classification,
        socialImpactScore: score,
        socialImpactRationale: rationale
      };
    });
  }
  ...
  getMockSuggestions() {
    return [
      ...
      },
      {
        ...
        icon: '📖',
        color: '#10b981',
        label: 'Add comprehensive docstring',
        explanation: 'Scholar: This function lacks documentation. Adding a descriptive docstring will improve code maintainability.',
        filePath: 'src/utils/formatters.js',
        rangeStart: { line: 25, ch: 0 },
        rangeEnd: { line: 25, ch: 0 },
        diff: {
          old: 'function formatDateTime(date, locale = "en-US") {',
          new: '/**\n * Formats a date object according to the specified locale\n * \n * @param {Date} date - The date object to format\n * @param {string} locale - The locale to use for formatting (default: "en-US")\n * @returns {string} The formatted date string\n */\nfunction formatDateTime(date, locale = "en-US") {'
        },
        impact: {
          type: 'quality',
          oldValue: '0%',
