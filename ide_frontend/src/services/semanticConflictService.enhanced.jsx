...
class EnhancedSemanticConflictService {
  ...
  detectSemanticConflicts(newInfo, existingInfo, newInfoContext, existingInfoContext) {
    try {
      ...
      // ---- Step 2: Check for semantic conflicts (same topic, different approaches) ----
      const semanticResult = this.checkSemanticConflicts(newInfo, existingInfo, newInfoContext, existingInfoContext);
  ...
  /**
   * Check for semantic conflicts (statements that address the same topic with conflicting approaches)
   * @param {string} newInfo - New statement
   * @param {string} existingInfo - Existing statement
   * @param {Object} newInfoContext - Context of the new information
   * @param {Object} existingInfoContext - Context of the existing information
   * @returns {Object} - Semantic conflict analysis
   */
  checkSemanticConflicts(newInfo, existingInfo, newInfoContext, existingInfoContext) {
    ...
    /**
     * Extract prescriptive terms from a statement
     * @param {string} text - Text to analyze
     * @returns {Array<string>} - List of prescriptive terms
     */
    extractPrescriptiveTerms(text) {
      // Simple implementation - extract prescriptive language patterns
      ...
      let prescriptiveTerms = [];
      
      for (const pattern of prescriptivePatterns) {
        pattern.lastIndex = 0; // Reset regex
        let match;
        while ((match = pattern.exec(text)) !== null) {
          if (match[1] && match[1].length > 2) { // Ensure it's a significant term
            prescriptiveTerms.push(match[0].toLowerCase());
          }
        }
      }
      
      return prescriptiveTerms.length > 0 ? prescriptiveTerms : [];
    }
    ...
    /**
     * Assess the potential impact of a conflict between two statements
     * @param {string} existing - Existing statement
     * @param {string} newInfo - New information
     * @param {Object} conflictAnalysis - Result of conflict analysis
     * @returns {Promise<Object>} - Impact assessment
     */
    async assessImpact(existing, newInfo, conflictAnalysis) {
      // Start with medium impact as default
      let level = this.impactLevels.MODERATE.id;
      let confidence = 0.5;
      let scope = [];
      
      try {
        // Use domain information to refine impact assessment
        if (conflictAnalysis.domainContext) {
          const { domains, priority } = conflictAnalysis.domainContext;
          
          // Domains with higher priority have higher impact
          if (domains.length > 0 && domains[0].priority > 0.7) {
            level = this.impactLevels.GLOBAL.id;
            confidence = domains[0].priority;
            scope = domains.map(d => d.name);
          } 
          // Many affected domains = higher impact
          else if (domains.length >= 3) {
            level = this.impactLevels.GLOBAL.id;
            confidence = Math.min(0.9, 0.5 + (domains.length * 0.1));
            scope = domains.map(d => d.name);
          }
          // Few affected domains = moderate impact
          else if (domains.length > 0) {
            level = this.impactLevels.MODERATE.id;
            confidence = priority;
            scope = domains.map(d => d.name);
          }
        }
        
        // Use severity information to adjust impact
        const severityLevel = this.getSeverityForType(conflictAnalysis.type);
        if (severityLevel === this.conflictTypes.DIRECT.severity) {
          // Direct conflicts have at least moderate impact
          level = level === this.impactLevels.MINIMAL.id ? 
            this.impactLevels.MODERATE.id : level;
        ...
        return {
          ...
          confidence,
          scope,
          description: this.getImpactDescription(level),
        ...
        return {
          ...
          confidence: 0.4,
          scope: [],
          description: this.getImpactDescription(this.impactLevels.MODERATE.id),
          error: error.message
        };
      }
    }
    ...
    /**
     * Get a human-readable description of an impact level
     * @param {string} level - Impact level ID
     * @returns {string} - Description
     */
    getImpactDescription(level) {
      for (const [id, details] of Object.entries(this.impactLevels)) {
        if (details.id === level) {
    ...
    /**
     * Calculate the semantic similarity between two statements
     ...
     */
    calculateSemanticSimilarity(statement1, statement2) {
    ...
    /**
     * Extract key concepts that are shared between two statements
     * @param {string} statement1 - First statement
     * @param {string} statement2 - Second statement
     * @returns {Array<string>} - List of shared concepts
     */
    extractSharedConcepts(statement1, statement2) {
      // Simple implementation using overlapping n-grams
      ...
      // Get both 1-grams and 2-grams
      const words1 = new Set(getNGrams(statement1, 1));
      const words2 = new Set(getNGrams(statement2, 1));
      const bigrams1 = new Set(getNGrams(statement1, 2));
      const bigrams2 = new Set(getNGrams(statement2, 2));
      
      // Find shared words (weighted higher for bigrams)
      const shared = [];
      
      // Add bigrams first (they're more significant)
      ...
      for (const word of words1) {
        if (words2.has(word) && !shared.some(concept => concept.includes(word))) {
          shared.push(word);
        }
      }
    ...
    /**
     * Detect semantic opposites in two statements that might indicate conflict
     * @param {string} statement1 - First statement
     * @param {string} statement2 - Second statement
     * @returns {Object} - Analysis of semantic opposites
     */
    detectSemanticOpposites(statement1, statement2) {
      // Enhanced semantic opposite pairs
      const semanticOppositePairs = [
        // Architecture/approach opposites
        ['monolithic', 'microservices'], ['centralized', 'distributed'],
        ['synchronous', 'asynchronous'], ['blocking', 'non-blocking'],
        ['stateful', 'stateless'], ['mutable', 'immutable'],
        ['eager', 'lazy'], ['strict', 'loose'],
        
        // Style opposites
        ['functional', 'object-oriented'], ['declarative', 'imperative'],
        ['simple', 'complex'], ['compact', 'verbose'],
        
        // Implementation opposites
        ['frontend', 'backend'], ['client-side', 'server-side'],
        ['compile-time', 'runtime'], ['development', 'production'],
        ['native', 'cross-platform']
      ];
      
      const words1 = statement1.toLowerCase().split(/\W+/);
      const words2 = statement2.toLowerCase().split(/\W+/);
      
      const foundPairs = [];
      let maxConfidence = 0;
      
      // Check for semantic opposite pairs
      for (const [term1, term2] of semanticOppositePairs) {
        const hasTerm1InStatement1 = words1.includes(term1);
        const hasTerm2InStatement1 = words1.includes(term2);
        const hasTerm1InStatement2 = words2.includes(term1);
        const hasTerm2InStatement2 = words2.includes(term2);
        
        // Check for opposite terms in opposing statements
        if ((hasTerm1InStatement1 && hasTerm2InStatement2) || 
            (hasTerm2InStatement1 && hasTerm1InStatement2)) {
          
          foundPairs.push([term1, term2]);
          
          // Calculate confidence - higher if terms are central to statements
          const position1 = hasTerm1InStatement1 ? 
            words1.indexOf(term1) / words1.length : 
            words1.indexOf(term2) / words1.length;
            
          const position2 = hasTerm1InStatement2 ? 
            words2.indexOf(term1) / words2.length : 
            words2.indexOf(term2) / words2.length;
            
          // Terms closer to middle of statement are more central
          const centrality1 = 1 - Math.abs(0.5 - position1) * 2;
          const centrality2 = 1 - Math.abs(0.5 - position2) * 2;
          
          const pairConfidence = 0.6 + (0.2 * (centrality1 + centrality2) / 2);
          maxConfidence = Math.max(maxConfidence, pairConfidence);
        }
      }
  ...
  export default new EnhancedSemanticConflictService();
