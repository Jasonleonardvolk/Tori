...
import {
  ...
  x2 as x,
  y
} from "./chunk-X7OBV3KU.js";
import "./chunk-PZ5AY32C.js";

// ../node_modules/.pnpm/@react-oauth+google@0.12.2_react-dom@17.0.2_react@17.0.2/node_modules/@react-oauth/google/dist/index.esm.js
function useLoadGsiScript(options = {}) {
  const { nonce, onScriptLoadSuccess, onScriptLoadError } = options;
  const [scriptLoadedSuccessfully, setScriptLoadedSuccessfully] = d(false);
  const onScriptLoadSuccessRef = A(onScriptLoadSuccess);
  onScriptLoadSuccessRef.current = onScriptLoadSuccess;
  const onScriptLoadErrorRef = A(onScriptLoadError);
...
function GoogleOAuthProvider({ clientId, nonce, onScriptLoadSuccess, onScriptLoadError, children }) {
  ...
  return Rn.createElement(GoogleOAuthContext.Provider, { value: contextValue }, children);
}
function useGoogleOAuth() {
  const context = x(GoogleOAuthContext);
  if (!context) {
...
function GoogleLogin({ onSuccess, onError, useOneTap, promptMomentNotification, type = "standard", theme = "outline", size = "large", text, shape, logo_alignment, width, locale, click_listener, containerProps, ...props }) {
  const btnContainerRef = A(null);
  const { clientId, scriptLoadedSuccessfully } = useGoogleOAuth();
  const onSuccessRef = A(onSuccess);
  onSuccessRef.current = onSuccess;
  const onErrorRef = A(onError);
  onErrorRef.current = onError;
  const promptMomentNotificationRef = A(promptMomentNotification);
  ...
  return Rn.createElement("div", { ...containerProps, ref: btnContainerRef, style: { height: containerHeightMap[size], ...containerProps === null || containerProps === void 0 ? void 0 : containerProps.style } });
}
...
function useGoogleLogin({ flow = "implicit", scope = "", onSuccess, onError, onNonOAuthError, overrideScope, state, ...props }) {
  ...
  const onErrorRef = A(onError);
  onErrorRef.current = onError;
  const onNonOAuthErrorRef = A(onNonOAuthError);
