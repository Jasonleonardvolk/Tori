...
console.log('🔍 Running tests with 20-second timeout...\n');

// Configuration
const TIMEOUT_MS = 20000; // 20 seconds
const EARLY_STOP_ERRORS = 50;
...
const timeoutId = setTimeout(() => {
  if (!killed) {
    console.log('\n⏰ TIMEOUT REACHED (20 seconds) - Stopping tests...');
    killed = true;
...
testProcess.on('close', (code) => {
  ...
  const patterns = {
    ...
    actualErrors: lines.filter(l => l.includes('× ') && !l.includes('mockMcp') && !l.includes('mock'))
  };
  
  // Write summary
  summaryStream.write('ALAN IDE Test Execution Summary (20s Run)\n');
  summaryStream.write('========================================\n\n');
  summaryStream.write(`Timestamp: ${new Date().toISOString()}\n`);
  summaryStream.write(`Duration: 20 seconds (timeout)\n`);
  ...
  summaryStream.write('\nFailing Test Suites:\n');
  patterns.failingSuites.slice(0, 10).forEach(suite => {
    summaryStream.write(`  ${suite}\n`);
  });
  
  summaryStream.write('\nActual Test Errors (non-mock):\n');
  patterns.actualErrors.slice(0, 10).forEach(error => {
    summaryStream.write(`  ${error}\n`);
  });
  ...
  summaryStream.write('3. Focus on the actual test errors listed above\n');
