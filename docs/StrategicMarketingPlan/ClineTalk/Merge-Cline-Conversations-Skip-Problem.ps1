# PowerShell script to merge Cline conversations while excluding the problematic file
...
$problemFolder = "1747599198541"  # Exclude this specific folder

Write-Host "Creating 10 chronological files with complete content from all api_conversation_history.json files..."
Write-Host "Excluding the problematic file from folder: $problemFolder"
...
$allFolders = Get-ChildItem -Path $sourceFolder -Directory | 
    Where-Object { $_.Name -ne $problemFolder } |  # Exclude the problem folder
    Sort-Object { [long]$_.Name }

$totalFolders = $allFolders.Count
Write-Host "Found $totalFolders folders to process (excluding the problematic one)."
    ...
    for ($i = 1; $i -le 10; $i++) {
        ...
        if ($format -eq "json") {
            ...
            $mergedData = @{
                ...
                "part" = "$i of 10"
                "folders_included" = @()
                "problem_folder_excluded" = $problemFolder
                "conversations" = @()
            }
...
# Created: $(Get-Date)
# Note: Excludes problematic folder $problemFolder which is being processed separately
...
Write-Host "Remember to process the problematic folder $problemFolder separately using the Process-Large-File script."
