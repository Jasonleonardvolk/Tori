...
async def lifespan(app: FastAPI):
    ...
    logger.info("🧠 Initializing Prajna - TORI's Voice, Language Model, and Reasoning System")
    
    try:
        # Load configuration
        config = PrajnaConfig()
        logger.info(f"📋 Prajna config loaded: {config.model_type}")
        ...
        logger.info("💾 Soliton Memory interface initialized")
        ...
        logger.info("🕸️ Concept Mesh API initialized")
        ...
        if getattr(config, 'enable_reasoning', True):
            ...
            logger.info("🧠 Prajna Reasoning Engine initialized")
        else:
            logger.info("⏭️ Reasoning Engine disabled")
        ...
        logger.info(f"🗣️ Prajna Language Model loaded: {config.model_type}")
...
async def prajna_answer_endpoint(request: PrajnaRequest):
    ...
    try:
        logger.info(f"🤔 Prajna received query: {request.user_query[:100]}...")
        
        # Step 1: Build enhanced context from memory systems with reasoning
        logger.info("🔍 Building enhanced context from TORI's memory systems...")
        ...
        if (request.enable_reasoning and reasoning_engine and 
            ...
            logger.info(f"🧠 Performing explicit reasoning: {request.reasoning_mode}")
        ...
        logger.info("🗣️ Invoking Prajna to synthesize answer...")
        ...
        logger.info("👽 Running alien overlay audit on Prajna's answer...")
        ...
        logger.info("👻 Analyzing ghost patterns in Prajna's response...")
        ...
        logger.info(f"✅ Prajna response complete - Trust: {trust_score:.2f}, Reasoning: {reasoning_triggered}, Time: {processing_time:.2f}s")
...
async def upload_pdf(file: UploadFile = File(...)):
    """
    🚀 ADMIN BULLETPROOF UPLOAD FOR PRAJNA WITH CONCEPT EXTRACTION
    ...
    try:
        ...
        logger.info(f"📤 [PRAJNA-UPLOAD] Admin uploading: {file.filename}")
        ...
        logger.info(f"📁 [PRAJNA-UPLOAD] Saving to: {dest_path}")
        ...
        logger.info(f"✅ [PRAJNA-UPLOAD] Admin upload successful!")
        logger.info(f"📏 [PRAJNA-UPLOAD] Size: {file_size_mb:.2f} MB")
        logger.info(f"⚡ [PRAJNA-UPLOAD] Upload time: {upload_time:.2f}s")
        
        # 🚨 NOW RUN CONCEPT EXTRACTION PIPELINE!
        ...
        if EXTRACTION_AVAILABLE:
            try:
                logger.info(f"🧬 [PRAJNA-UPLOAD] Starting concept extraction...")
                ...
                if extraction_result.get("status") == "success":
                    ...
                    logger.info(f"🎉 [PRAJNA-UPLOAD] Extraction successful!")
                    logger.info(f"📊 [PRAJNA-UPLOAD] Extracted {concept_count} concepts")
                    logger.info(f"⚡ [PRAJNA-UPLOAD] Extraction time: {extraction_time:.2f}s")
                    
                    # Log some concept names for debugging
                    if concept_names:
                        sample_concepts = concept_names[:5]
                        logger.info(f"🔍 [PRAJNA-UPLOAD] Sample concepts: {sample_concepts}")
                else:
                    extraction_status = "failed"
                    error_msg = extraction_result.get("error_message", "Unknown extraction error")
                    logger.error(f"❌ [PRAJNA-UPLOAD] Extraction failed: {error_msg}")
                    
            except Exception as extraction_error:
                logger.error(f"❌ [PRAJNA-UPLOAD] Extraction exception: {extraction_error}")
                extraction_status = "error"
                
        else:
            logger.warning(f"⚠️ [PRAJNA-UPLOAD] Extraction pipeline not available (fallback mode)")
        ...
        logger.info(f"🏁 [PRAJNA-UPLOAD] Complete! Total time: {total_time:.2f}s")
        ...
        logger.error(f"❌ [PRAJNA-UPLOAD] Admin upload failed: {e}")
        raise HTTPException(
            status_code=500,
