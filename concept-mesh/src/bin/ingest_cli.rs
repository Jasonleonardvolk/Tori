...
use concept_mesh::agents::orchestrator::{create_orchestrator, IngestDocument, OrchestratorConfig};
use concept_mesh::cbd::{CBDConfig, ConceptBoundaryDetector};
use concept_mesh::cli::adapter::{CliAdapter, create_ingest_command, create_status_command};
use concept_mesh::mesh::InMemoryMesh;
use concept_mesh::psiarc::{PsiarcManager, PsiarcOptions};
use concept_mesh::ui::genesis_bridge::{GenesisBridge, OscillatorBloom, create_genesis_ui_bridge};
use concept_mesh::auth::session::get_current_session;
use concept_mesh::ingest::pdf_user_attribution::{IngestSourceInfo, extract_tags_from_content};
...
impl IngestStats {
    ...
    fn print_success(&self, use_color: bool) {
        ...
        println!("{} Ingested \"{}\" as {} (persona: {})",
            "📥".bright_green(),
            filename.bright_white().bold(),
            self.user_id.as_deref().unwrap_or("anonymous").bright_blue(),
            self.persona_id.as_deref().unwrap_or("default").bright_yellow());
            
        println!("{} ConceptDiffs: {} | Phase regions: {}",
            "🧠".bright_cyan(),
...
async fn main() -> Result<(), String> {
    ...
    let _genesis_bridge = create_genesis_ui_bridge("ingest-cli", Arc::clone(&mesh), Arc::clone(&bloom)).await?;
    
    // Execute command
    match args.command {
        Command::Ingest { file, document_type, title, tags, source_title, persona, auto_tag } => {
            ...
            // Add source info
            document = document.with_metadata("source_info", serde_json::json!(source_info.to_metadata()));
            ...
            let psiarc_filename = format!("ψ-{}.psiarc", chrono::Utc::now().format("%Y-%m-%dT%H:%M:%S"));
        ...
        Command::Status { job_id } => {
            ...
            if !args.no_color {
                println!("{} Status: {}", "ℹ️".bright_blue(), status.bright_white());
            } else {
                println!("Status: {}", status);
            }
        }
        Command::Watch { dir, pattern, auto_tag } => {
            ...
            if !args.no_color {
                println!("{} Watching for files in: {}", "👁️".bright_magenta(), dir.display().to_string().bright_white());
                println!("{} File pattern: {}", "🔍".bright_yellow(), pattern.bright_white());
                println!("{} Press {} to stop", "ℹ️".bright_blue(), "Ctrl+C".bright_red());
            } else {
            ...
            watcher.await.map_err(|e| format!("File watcher error: {}", e))?;
        }
    }
