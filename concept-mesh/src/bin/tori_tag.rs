...
use concept_mesh::{self, ConceptId, LargeConceptNetwork};
use concept_mesh::auth::session::{get_current_session, Session};
use concept_mesh::diff::{ConceptDiff, ConceptDiffBuilder, UserContextExt};
use concept_mesh::concept_trail::{ConceptTrail, ConceptTrailFilter, create_trail_from_psiarc};
use concept_mesh::psiarc::{PsiArcReader, PsiarcHeader};
...
#[derive(Parser, Debug)]
#[clap(name = "tori-tag", about = "Add, update, or remove tags for concepts post-ingest")]
struct Args {
    #[clap(subcommand)]
    command: Command,
...
fn find_matching_concepts(
    ...
    match target_type {
        ...
        "document" => {
            ...
            if let Some(path) = psiarc_path {
                ...
                return Err("PsiArc file or directory is required for document target type".to_string());
            }
        },
        "source" => {
            ...
            if let Some(path) = psiarc_path {
                ...
                return Err("PsiArc file or directory is required for source target type".to_string());
            }
        },
        _ => {
    ...
    if matching_concept_ids.is_empty() {
        return Err(format!("No concepts found matching {} '{}'", target_type, target));
    }
...
fn update_tags(
    ...
    let concept_data = if let Some(path) = psiarc_path {
                    ...
                    Ok(frames) => {
                        ...
                        for frame in frames {
                            for op in &frame.diff.ops {
                                if let concept_mesh::diff::Op::Create { id, concept_type, properties } = op {
                                    if concept_ids.contains(id) {
                                        ...
                                        let document_id = properties.get("document_id")
                                            .and_then(|v| v.as_str())
                                            .map(|s| s.to_string());
                                            
                                        let source_title = properties.get("source_title")
                                            .and_then(|v| v.as_str())
                                            .map(|s| s.to_string());
                                            
                                        let existing_tags = properties.get("tags")
                                        ...
                                        data.insert(id.clone(), (concept_type.clone(), title, document_id, source_title, existing_tags));
                                    }
                                }
                            }
                        }
        ...
        for concept_id in concept_ids {
            ...
            if let Some(concept) = lcn.get_concept(concept_id) {
                ...
                let title = concept.properties.get("title")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                    
                let document_id = concept.properties.get("document_id")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                    
                let source_title = concept.properties.get("source_title")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                    
                let existing_tags = concept.properties.get("tags")
                    .and_then(|v| v.as_array())
                    .map(|arr| {
                        arr.iter()
                            .filter_map(|v| v.as_str().map(|s| s.to_string()))
                ...
                data.insert(concept_id.clone(), (concept_type, title, document_id, source_title, existing_tags));
            } else {
    ...
    for concept_id in concept_ids {
        if let Some((concept_type, title, document_id, source_title, existing_tags)) = concept_data.get(concept_id) {
...
fn print_results(results: &[TagUpdateResult], use_color: bool) {
    ...
    // Print header
    println!("{} Tag Update Results", "🔖".bright_yellow());
    println!();
    ...
    for result in results {
        ...
        // Print concept info
        println!("{} {} ({})", "📄".bright_blue(), title.bright_white().bold(), concept_type.bright_cyan());
    ...
    // Print summary
    let success_count = results.iter().filter(|r| r.status.starts_with("Success")).count();
    println!("{} Updated {} of {} concepts", 
        if success_count == results.len() { "✅".bright_green() } else { "⚠️".bright_yellow() },
...
async fn main() -> Result<(), String> {
    ...
    // Execute command
    match args.command {
        Command::Add { target, tags, target_type, psiarc, recursive } => {
        ...
        Command::Remove { target, tags, target_type, psiarc, recursive } => {
        ...
        Command::List { target, target_type, psiarc, format } => {
