...
use concept_mesh::auth::{
    oauth::{OAuthClient, load_oauth_credentials, init_oauth_providers},
    user::{UserStore, create_user_store},
    session::{SessionManager, create_session_manager, get_current_session},
    cli::{handle_cli_auth, parse_cli_args},
};
use concept_mesh::agents::persona_packs::{
    initialize_registry, update_packs_for_current_persona
};
...
fn initialize_oauth(config_file: &PathBuf) -> Result<()> {
    info!("Initializing OAuth providers from {}", config_file.display());
    ...
    if config_file.exists() {
        ...
        warn!("OAuth configuration file not found: {}", config_file.display());
...
fn initialize_user_store(user_store_dir: &PathBuf) -> Result<()> {
    ...
    std::env::set_var("CONCEPT_MESH_USER_STORE_DIR", user_store_dir.to_string_lossy().to_string());
...
fn handle_cli_authentication() -> Result<()> {
    ...
    if args.impersonate.is_some() {
        ...
        info!("CLI authentication successful: user={}, persona={}",
...
fn restore_previous_session() -> Result<()> {
    ...
    match session_manager.restore_previous_session() {
        Ok(Some(session)) => {
            info!("Restored previous session: user={}, persona={}",
                  session.user.name.as_deref().unwrap_or("Unknown"),
                  session.persona.mode.as_str());
            Ok(())
        },
...
fn update_agent_packs() -> Result<()> {
    ...
    if let Some(session) = get_current_session() {
        info!("Updating agent packs for persona: {}", session.persona.mode.as_str());
...
pub fn main() -> Result<()> {
    ...
    if let Some(session) = get_current_session() {
        ...
        println!("  User: {}", session.user.name.unwrap_or_else(|| session.user.concept_id.clone()));
