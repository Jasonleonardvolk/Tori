...
async fn main() -> Result<(), String> {
    ...
    while start_time.elapsed() < Duration::from_secs(args.time) {
        ...
        if args.simulate_perturbation && !perturbation_applied && start_time.elapsed() > Duration::from_secs(args.time / 2) {
        ...
        if Instant::now() >= next_stats_time {
            ...
            info!("  Avg. processing time: {:.3} μs", bus_stats.avg_processing_time_us);
            info!("  Max processing time: {} μs", bus_stats.max_processing_time_us);
            info!("  Tick jitter: {} μs", bus_stats.tick_jitter_us);
            ...
            let avg_group_diff = if count > 0 { sum_diff / count as f64 } else { 0.0 };
