...
impl ConceptBoundaryDetector {
    ...
    fn extract_concept_pack(&mut self, pos: usize, reason: BoundaryReason) -> ConceptPack {
        ...
        pack.metadata.insert("word_count".to_string(), serde_json::json!(content.split_whitespace().count()));
    ...
    fn create_concept_pack(&mut self, reason: BoundaryReason) -> ConceptPack {
        ...
        // Add metadata
        pack.metadata.insert("word_count".to_string(), serde_json::json!(pack.content.split_whitespace().count()));
    ...
    fn detect_koopman_inflection(&self) -> bool {
        ...
        let all_concepts = current.keys().chain(previous.keys()).collect::<std::collections::HashSet<_>>();
...
mod tests {
    ...
    fn test_concept_pack_creation() {
        let pack = ConceptPack::new("test_1", "This is a test", 0, 14, BoundaryReason::ExplicitMarker);
