...
//!
//! This module provides validation utilities for ConceptDiffs stored in ψarc logs.
...
#[derive(Parser, Debug)]
#[clap(name = "psidiff-validate", about = "Validate ConceptDiffs in ψarc logs")]
...
pub async fn validate_psiarc(
    ...
    let replay_result = replay_psiarc(
        input_path.as_ref(),
        false,
        None,
        None,
    ...
    if let Some(ref_path) = reference_graph {
        ...
        // Update metrics
        let node_metric = result.metrics.iter_mut().find(|m| m.name == "Node Count").unwrap();
        node_metric.expected = format!("{}", ref_node_count);
        node_metric.passed = replay_result.node_count == ref_node_count;
        
        let edge_metric = result.metrics.iter_mut().find(|m| m.name == "Edge Count").unwrap();
...
pub async fn execute_validate_cmd(args: ValidateArgs) -> Result<(), String> {
    ...
    if let Some(ref graph_path) = args.graph {
        if !graph_path.exists() {
            return Err(format!("Reference graph not found: {}", graph_path.display()));
        }
    }
    
    // Run validation
    let result = validate_psiarc(
        args.input.clone(),
        args.graph.as_ref(),
        args.tags_only,
    )
    ...
    if !result.passed {
        if let Some(failure_idx) = result.first_failure {
            return Err(format!("Validation failed at diff {} in psiarc file", failure_idx));
        } else {
...
fn print_validation_summary(result: &ValidationResult) {
    ...
    for metric in &result.metrics {
        ...
        let diff_percent = metric.diff_percent.map_or("N/A".to_string(), |d| format!("{:.2}%", d));
    ...
    // Summary
    println!("\nSummary:");
    println!("  Diffs Processed: {}", result.replay_result.diffs_processed);
    println!("  Processing Time: {} ms", result.processing_time_ms);
    println!("  Overall Result:  {}", if result.passed {
        "✅ PASSED"
    } else {
        "❌ FAILED"
    });
