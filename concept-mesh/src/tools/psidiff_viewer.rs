...
//! ψarc (psiarc) logs, allowing users to inspect ConceptDiff transactions,
...
use crate::psiarc::{PsiarcReader, PsiarcManager};
...
/// Command line arguments for the ψdiff-viewer
#[derive(Parser, Debug)]
#[clap(name = "psidiff-viewer", about = "Visualize ConceptDiff transactions from ψarc logs")]
struct Args {
...
/// Subcommands for the ψdiff-viewer
#[derive(Subcommand, Debug)]
enum Command {
    /// View a ψarc log file
    View {
        /// Path to the ψarc log file
        #[clap(name = "FILE")]
        file: PathBuf,
    ...
    /// List all ψarc log files in the directory
    List {
        /// Directory containing ψarc logs
        #[clap(name = "DIR", default_value = "logs")]
        dir: PathBuf,
    },
    
    /// Replay a ψarc log to a fresh LCN
    Replay {
        /// Path to the ψarc log file
        #[clap(name = "FILE")]
        file: PathBuf,
...
fn format_diff(diff: &ConceptDiff) -> String {
    ...
    for (i, op) in diff.ops.iter().enumerate() {
        let op_str = match op {
            ...
            Op::Link { source, target, rel_type, .. } => {
                format!("Link {}--[{}]-->{}", source, rel_type, target)
            }
            Op::Unlink { source, target, rel_type } => {
...
fn visualize_activations(diffs: &[ConceptDiff]) -> String {
    ...
    for diff in diffs {
        for op in &diff.ops {
            match op {
                Op::Create { node, node_type, .. } if node_type == "Concept" => {
                    concepts.insert(node.clone());
                }
                Op::Link { source, target, rel_type, .. } if rel_type == "HAS_CONCEPT" => {
    ...
    for concept in concepts {
        ...
        for diff in diffs {
            ...
            for op in &diff.ops {
                match op {
                    Op::Create { node, node_type, .. } if node_type == "Concept" && node == &concept => {
                        active = true;
                    }
                    Op::Link { target, rel_type, .. } if rel_type == "HAS_CONCEPT" && target == &concept => {
                        active = true;
                    }
                    _ => {}
                }
            ...
            match op {
                ...
                Op::Link { source, target, .. } | Op::Unlink { source, target, .. } => {
                    if nodes.contains(source) || nodes.contains(target) {
                        ...
                        related_node = if nodes.contains(source) { source.clone() } else { target.clone() };
                        operation = format!("{:?}", op);
                        break;
                    }
                }
                Op::Bind { parent, node, .. } => {
                    if nodes.contains(parent) || nodes.contains(node) {
                        ...
                        related_node = if nodes.contains(parent) { parent.clone() } else { node.clone() };
                        operation = format!("{:?}", op);
                        break;
                    }
                }
...
async fn handle_view_command(
    ...
    // Open psiarc file
    let mut reader = PsiarcReader::open(&file)
    ...
    while let Ok(Some(diff)) = reader.next_diff() {
        ...
        if let Some(ref target_filter) = target {
            if diff.targets.as_ref().map_or(true, |t| !t.contains(target_filter)) {
                continue;
            }
        }
        ...
        if let Some(ref op_filter) = op {
            let has_matching_op = diff.ops.iter().any(|op| {
                match op {
...
async fn handle_replay_command(
    ...
    println!("LCN status: GENESIS complete = {}", lcn.is_genesis_complete());
