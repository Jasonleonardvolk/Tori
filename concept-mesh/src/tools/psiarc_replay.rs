...
//!
//! This module provides a replay engine for ConceptDiffs stored in ψarc logs.
...
#[derive(Parser, Debug)]
#[clap(name = "psiarc-replay", about = "Replay ConceptDiffs from ψarc logs")]
...
fn print_replay_summary(input_path: &Path, result: &ReplayResult) {
    ...
    println!("GENESIS Found:     {}", if result.genesis_encountered { "Yes" } else { "No" });
