...
pub use phase_event_bus::{
    PhaseEvent, PhaseEventBus, PhaseEventType, PhaseEventData, SubscriptionPattern,
    PhaseAgent, create_phase_event_bus,
};
...
impl InMemoryMesh {
    ...
    pub async fn initialize(&mut self, bus_id: &str) -> Result<(), String> {
        ...
        if self.event_bus.is_none() {
            let bus = create_phase_event_bus(bus_id).await?;
    ...
    pub async fn start_all(&mut self) -> Result<(), String> {
        for node in &mut self.nodes {
            ...
            let result = {
                let node = Arc::get_mut(&mut node_clone)
                    .ok_or_else(|| format!("Failed to get mutable reference to node {}", node.id()))?;
