...
use crate::diff::{ConceptDiff, ConceptDiffRef, Op, NodeId};
...
impl LargeConceptNetwork {
    ...
    pub fn apply_diff(&self, diff: &ConceptDiff) -> Result<(), String> {
        ...
        for op in &diff.ops {
            match op {
                Op::Create { node, node_type, properties } => {
                ...
                Op::Link { source, target, rel_type, properties } => {
    ...
    pub fn get_relationships(&self, source_id: &NodeId, target_id: &NodeId) -> Vec<ConceptRelationship> {
        let node_indices = self.node_indices.read().unwrap();
        let graph = self.graph.read().unwrap();
        
        if let (Some(source_idx), Some(target_idx)) = (node_indices.get(source_id), node_indices.get(target_id)) {
...
mod tests {
    ...
    fn test_concept_creation() {
        ...
        assert_eq!(concept.get_property("name").unwrap().as_str().unwrap(), "Test Concept");
    ...
    fn test_lcn_create_node() {
        ...
        assert_eq!(node.get_property("name").unwrap().as_str().unwrap(), "Test Node");
    }
    ...
    fn test_lcn_create_relationship() {
        ...
        // Create nodes
        lcn.create_node(&"source".to_string(), "TestType", &HashMap::new()).unwrap();
        lcn.create_node(&"target".to_string(), "TestType", &HashMap::new()).unwrap();
        ...
        assert_eq!(
            relationships[0].properties.get("weight").unwrap().as_f64().unwrap(),
            0.5
        );
    }
    ...
    fn test_lcn_apply_diff() {
        ...
        assert_eq!(lcn.get_relationships(&"node1".to_string(), &"node2".to_string()).len(), 1);
        assert_eq!(
