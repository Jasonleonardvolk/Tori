...
impl LyapunovCertifier {
    ...
    pub fn certify_plan(&self, plan: &PlanGraph) -> CertificationResult {
        debug!("Certifying plan with {} nodes and {} edges", plan.nodes.len(), plan.edges.len());
        ...
        if instability > self.config.max_instability {
            cert_ok = false;
            reason = Some(format!("Instability risk: V̇ = {:.3} > threshold {:.3}", 
                                 instability, self.config.max_instability));
            recommendation = Some("Reduce the branching factor of the plan or add stabilizing constraints".to_string());
        }
        
        // Check complexity
        if complexity > self.config.max_complexity {
            cert_ok = false;
            reason = Some(format!("Plan too complex: complexity = {:.3} > threshold {:.3}", 
                                 complexity, self.config.max_complexity));
            recommendation = Some("Simplify the plan by reducing the number of steps or parallel paths".to_string());
        }
        ...
        if confidence < self.config.min_confidence {
            if self.config.strict_mode {
                cert_ok = false;
                reason = Some(format!("Low confidence: confidence = {:.3} < threshold {:.3}", 
                                     confidence, self.config.min_confidence));
                recommendation = Some("Add more constraints or explicit safety checks to the plan".to_string());
            } else {
                // In non-strict mode, low confidence is just a warning
                warn!("Low confidence in plan stability: {:.3} < threshold {:.3}", 
                     confidence, self.config.min_confidence);
            }
        }
    ...
    fn calculate_plan_complexity(&self, plan: &PlanGraph) -> f64 {
        ...
        let max_branches = plan.nodes.iter()
            .map(|node| {
                plan.edges.iter()
        ...
        debug!("Plan complexity: {:.3} (nodes={}, edges={}, max_branches={})",
    ...
    fn calculate_stability_derivative(&self, plan: &PlanGraph) -> f64 {
        ...
        let unstable_nodes = plan.nodes.iter()
            .filter(|node| {
                let out_edges = plan.edges.iter()
                    .filter(|edge| edge.source == node.id)
                    .count();
        ...
        let instability = (0.4 * node_factor + 0.4 * cycle_factor + 0.2 * isolation_factor).min(1.0);
        
        debug!("Plan instability (V̇): {:.3} (unstable_nodes={}, cycles={}, components={})",
    ...
    fn calculate_confidence(&self, plan: &PlanGraph) -> f64 {
        ...
        let detail_factor = plan.nodes.iter()
            .map(|node| node.attributes.len())
            .sum::<usize>() as f64 / (plan.nodes.len().max(1) * 5) as f64;
    ...
    fn detect_cycles(&self, plan: &PlanGraph) -> usize {
        ...
        for edge in &plan.edges {
            adj_list.entry(edge.source.clone())
    ...
    fn count_components(&self, plan: &PlanGraph) -> usize {
        ...
        for edge in &plan.edges {
            ...
            adj_list.entry(edge.target.clone())
                .or_insert_with(Vec::new)
    ...
    pub fn create_plan_rejected_diff(&self, plan_id: &str, result: &CertificationResult) -> ConceptDiff {
