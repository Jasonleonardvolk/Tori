...
impl Orchestrator {
    ...
    fn start_background_task(&self, mut cmd_rx: Receiver<OrchestratorCommand>) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            if let Ok(id) = mesh_node
                .subscribe(
                    vec![Pattern::new("INGEST_REQUEST")],
                    |diff| Self::handle_ingest_request_diff(diff, job_queue.clone(), cmd_tx.clone()),
                )
                .await
            {
    ...
    async fn handle_ingest_request_diff(
        ...
        if let Some(metadata) = diff.metadata.get("ingest_request") {
            if let Ok(doc) = serde_json::from_value::<IngestDocument>(metadata.clone()) {
                let _ = cmd_tx
                    .send(OrchestratorCommand::QueueDocument(
                        doc,
                        mpsc::channel(1).0,
                    ))
                    .await;
    ...
    async fn process_concept_pack_internal(
        ...
        properties.insert("boundary_reason".to_string(), serde_json::json!(format!("{:?}", pack.boundary_reason)));
        ...
        // Create node
        diff_builder = diff_builder.create(
            &pack_node_id,
            "ConceptPack",
            properties,
        );
        ...
        if job_id != "unknown" {
            diff_builder = diff_builder.bind(
                &job_id,
                &pack_node_id,
    ...
    fn create_job_queued_diff(job: &IngestJob) -> ConceptDiff {
        ...
        // Create job node
        let mut properties = HashMap::new();
        properties.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        properties.insert("document_type".to_string(), serde_json::json!(job.document.document_type.clone()));
        ...
        if !job.document.tags.is_empty() {
            properties.insert("tags".to_string(), serde_json::json!(job.document.tags.clone()));
        }
        ...
        payload.insert("job_id".to_string(), serde_json::json!(job.document.id.clone()));
        payload.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        
        diff_builder = diff_builder.signal(
            "JOB_QUEUED",
            None,
            payload,
        );
    ...
    fn create_job_processing_diff(job: &IngestJob) -> ConceptDiff {
        ...
        properties.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        properties.insert("updated_at".to_string(), serde_json::json!(job.updated_at));
        properties.insert("percent_complete".to_string(), serde_json::json!(job.percent_complete));
        
        diff_builder = diff_builder.update(
            &job.document.id,
            properties,
        );
        ...
        payload.insert("job_id".to_string(), serde_json::json!(job.document.id.clone()));
        payload.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        payload.insert("percent_complete".to_string(), serde_json::json!(job.percent_complete));
    ...
    fn create_job_completed_diff(job: &IngestJob) -> ConceptDiff {
        ...
        properties.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        properties.insert("updated_at".to_string(), serde_json::json!(job.updated_at));
        properties.insert("completed_at".to_string(), serde_json::json!(job.completed_at));
        properties.insert("percent_complete".to_string(), serde_json::json!(job.percent_complete));
        
        if !job.concept_ids.is_empty() {
            properties.insert("concept_ids".to_string(), serde_json::json!(job.concept_ids.iter().cloned().collect::<Vec<_>>()));
        }
        ...
        payload.insert("job_id".to_string(), serde_json::json!(job.document.id.clone()));
        payload.insert("status".to_string(), serde_json::json!(job.status.to_string()));
        payload.insert("concept_count".to_string(), serde_json::json!(job.concept_ids.len()));
...
mod tests {
    ...
    async fn test_orchestrator_creation() {
        ...
        let lcn = Arc::new(LargeConceptNetwork::new());
        let result = Orchestrator::new("test-orchestrator", mesh, lcn, OrchestratorConfig::default()).await;
    ...
    async fn test_queue_document() {
        let orchestrator = create_orchestrator("test-orchestrator", None).await.unwrap();
    ...
    async fn test_genesis_event() {
        ...
        let orchestrator = Orchestrator::new("test-orchestrator", Arc::clone(&mesh), Arc::clone(&lcn), config).await.unwrap();
