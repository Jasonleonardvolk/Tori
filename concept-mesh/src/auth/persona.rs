...
impl PersonaMode {
    ...
    pub fn description(&self) -> String {
        match self {
            ...
            Self::Glyphsmith => "ELFIN compiler, visual expressiveness, and glyph editing".to_string(),
            Self::MemoryPruner => "Timeline reflection, psiarc editing, and memory organization".to_string(),
            Self::Researcher => "AV/hologram ingestion, psiarc emission, and knowledge acquisition".to_string(),
...
impl PersonaStore {
    ...
    pub fn get_persona(&mut self, user_id: &str, mode: &PersonaMode) -> Result<Persona, std::io::Error> {
    ...
    pub fn list_personas_for_user(&mut self, user_id: &str) -> Result<Vec<Persona>, std::io::Error> {
    ...
    /// Delete a persona
    pub fn delete_persona(&mut self, user_id: &str, mode: &PersonaMode) -> Result<(), std::io::Error> {
        let file_name = format!("{}_{}.json", user_id, mode.as_str());
        let persona_path = self.path.join(&file_name);
    ...
    pub fn get_or_create_persona(&mut self, user_id: &str, mode: PersonaMode) -> Result<Persona, std::io::Error> {
        match self.get_persona(user_id, &mode) {
