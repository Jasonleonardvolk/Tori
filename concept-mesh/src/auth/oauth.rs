...
impl OAuthClient {
    ...
    pub fn authorization_url(&self, credentials: &OAuthCredentials) -> Result<String, OAuthError> {
        let mut url = match credentials.provider {
            OAuthProvider::Auth0 => {
                let domain = credentials.auth0_domain()
                    .ok_or_else(|| OAuthError::InvalidCredentials("Missing Auth0 domain".to_string()))?;
...
pub fn load_oauth_credentials(provider_name: &str) -> Result<OAuthCredentials, OAuthError> {
    let provider = OAuthProvider::from_str(provider_name)
        .map_err(|e| OAuthError::InvalidCredentials(e))?;
    ...
    let redirect_uri = std::env::var(&redirect_uri_key)
        .unwrap_or_else(|_| {
...
pub fn github_oauth_config() -> OAuthCredentials {
    OAuthCredentials::new(
        ...
        &std::env::var("GITHUB_REDIRECT_URI").unwrap_or("http://localhost:8000/auth/callback".to_string()),
    )
}
...
pub fn google_oauth_config() -> OAuthCredentials {
    OAuthCredentials::new(
        ...
        &std::env::var("GOOGLE_REDIRECT_URI").unwrap_or("http://localhost:8000/auth/callback".to_string()),
    )
}
...
pub fn apple_oauth_config() -> OAuthCredentials {
    OAuthCredentials::new(
        ...
        &std::env::var("APPLE_REDIRECT_URI").unwrap_or("http://localhost:8000/auth/callback".to_string()),
    )
}
...
pub fn discord_oauth_config() -> OAuthCredentials {
    OAuthCredentials::new(
        ...
        &std::env::var("DISCORD_REDIRECT_URI").unwrap_or("http://localhost:8000/auth/callback".to_string()),
    )
}
...
pub fn auth0_oauth_config() -> Result<OAuthCredentials, OAuthError> {
    ...
    Ok(OAuthCredentials::new(
        ...
        &std::env::var("AUTH0_REDIRECT_URI").unwrap_or("http://localhost:8000/auth/callback".to_string()),
