...
use super::{User, Persona, PersonaMode, Session, set_current_session};
...
pub fn handle_cli_auth(matches: &ArgMatches) -> Option<Session> {
    if let Some(username) = matches.get_one::<String>("impersonate") {
        ...
        let mode_str = matches.get_one::<String>("persona")
            .map(|s| s.as_str())
        ...
        info!("Impersonating user: {} as {}", username, mode.display_name());
...
pub fn add_cli_auth_args(app: Command) -> Command {
    ...
    .arg(
        Arg::new("persona")
            ...
            .value_name("PERSONA")
            .value_parser(["creative_agent", "glyphsmith", "memory_pruner", "researcher"])
...
pub fn check_env_impersonation() -> Option<Session> {
    ...
    info!("Impersonating user from environment: {} as {}", 
          username, mode.display_name());
...
mod tests {
    ...
    fn test_cli_impersonation() {
        ...
        let matches = app.clone()
            .try_get_matches_from(vec!["test", "--impersonate", "testuser", "--persona", "glyphsmith"])
            .unwrap();
