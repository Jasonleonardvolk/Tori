...
impl User {
    ...
    pub fn new(provider: &str, provider_id: &str, email: Option<&str>, name: Option<&str>, avatar_url: Option<&str>) -> Self {
    ...
    pub fn display_name(&self) -> String {
        self.name.clone()
            .or_else(|| self.email.clone().map(|e| e.split('@').next().unwrap_or("").to_string()))
...
impl UserStore {
    ...
    pub fn get_user_by_provider(&mut self, provider: &str, provider_id: &str) -> Result<User, UserError> {
        ...
        Err(UserError::NotFound(format!("Provider {}:{}", provider, provider_id)))
    }
...
mod tests {
    ...
    fn test_user_to_concept_diff() {
        ...
        if let crate::diff::Operation::Create { id, concept_type, attributes } = &diff.operations[0] {
            ...
            assert_eq!(attributes.get("avatar_url").unwrap(), "https://example.com/avatar.png");
        } else {
