...
impl CliAdapter {
    ...
    pub async fn new(orchestrator: Arc<Orchestrator>, mesh: Arc<dyn Mesh>) -> Result<Self, String> {
        // Create mesh node
                    ...
                    Ok(mut entries) => {
                        while let Ok(Some(entry)) = entries.next_entry().await {
                            ...
                            if path.is_file() && path.to_string_lossy().contains(&pattern) {
                                ...
                                if let Err(e) = cmd_tx.send(CliCommand::QueueDocument(path.clone(), tx)).await {
                    ...
                    match cmd {
                        CliCommand::QueueDocument(path, resp_tx) => {
                            ...
                            match Self::create_document_from_file(&path).await {
                                ...
                                Err(e) => {
                                    let _ = resp_tx.send(Err(format!("Failed to read file: {}", e))).await;
                                }
                            }
                        }
...
pub async fn create_status_command(
    adapter: &CliAdapter,
    job_id: &str,
) -> Result<String, String> {
...
pub async fn create_metrics_command(
    adapter: &CliAdapter,
...
mod tests {
    ...
    async fn test_create_document_from_file() {
        ...
        // Create document
        let doc = CliAdapter::create_document_from_file(file.path()).await.unwrap();
