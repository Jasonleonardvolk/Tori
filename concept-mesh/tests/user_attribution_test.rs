...
use concept_mesh::auth::{
    User, PersonaMode, Session, set_current_session,
};
use concept_mesh::diff::{
    ConceptDiff, ConceptDiffBuilder, UserContextExt,
    user_context::add_user_context,
};
use concept_mesh::psiarc::{
    PsiArc, PsiArcBuilder, PsiArcReader, PsiArcConfig,
};
...
fn test_multi_user_replay() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for frame in reader.read_all_frames()? {
        ...
        let diff = &frame.diff;
        if diff.frame_id > 0 { // Skip genesis frame
            assert!(diff.metadata.contains_key("user_id"));
            assert!(diff.metadata.contains_key("persona_id"));
