...
interface SpectralDashboardProps {
  showHistory?: boolean;
 ...
 */
const SpectralDashboard: React.FC<SpectralDashboardProps> = ({
  showHistory = true,
  historyLength = 20,
  showAlerts = true,
  className = '',
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'history' | 'alerts'>('overview');
  ...
  const stats = useMemo(() => {
    ...
    let coherenceTrend: 'up' | 'down' | 'stable' = 'stable';
    if (history.length >= 2) {
  ...
  const handleAcknowledgeAlert = (id: string, event: React.MouseEvent) => {
